...
class TestDataSetup(unittest.TestCase):
    ...
    def test_load_association_data(self):
        ...
        # Missing SE file
        self.assertRaises(ValueError, ds.load_association_data, path_dir = "./data/TestData/",
        ...
        self.assertRaises(ValueError, ds.load_association_data, path_dir = "./TestData/",
                                        eff_fname = "unstdBeta_df",
                                        exp_fname = "Beta_EXP.csv")
    def test_compute_pca(self):
        ...
        data = ds.load_association_data(path_dir = "./data/TestData/",
                                        eff_fname = "unstdBeta_df.csv",
                                        exp_fname = "Beta_EXP.csv")
        pca = ds.compute_pca(data["eff_df"])
        ...
        self.assertRaises(TypeError, ds.compute_pca, data["eff_df"].to_numpy())
    def test_calc_sum_sq(self):
        ...
        * missing column from data raises ValueError
        ...
        sum_sqs = ds.calc_sum_sq(cn, dummy_clusts)
        # Check the output is a float
        self.assertTrue(isinstance(sum_sqs, float))
        # Check failure for invalid cluster number
        self.assertRaises(IndexError, ds.calc_sum_sq, data = dummy_clusts, c_num = cnums + 1)
        # Check failure for missing data column
        dummy_clusts_missing = pd.DataFrame(index = data["eff_df"].index,
                                    data = {"clust_num": rand_nums})
        self.assertRaises(ValueError, ds.calc_sum_sq, data = dummy_clusts_missing, c_num = cnums - 1 )
        # Check failure for input not as Dataframe   
        self.assertRaises(TypeError, ds.calc_sum_sq, data = dummy_clusts.to_numpy(), c_num = cnums - 1) 
    def test_dist_met(self):
        ...
        * If metric is not a value option raises ValueError
        ...
        data = ds.load_association_data(path_dir = "./data/TestData/",
                                        ...
                                        exp_fname = "Beta_EXP.csv")
        met = "Euclidean"
        dist_met = ds.mat_dist(data["eff_df"], met)
        # Check output type
        self.assertTrue(isinstance(dist_met, np.ndarray))
        # Check if data is not a Dataframe raises TypeError
        self.assertRaises(TypeError, ds.mat_dist, data["eff_df"].to_numpy(), met = met)
        # Check if met is not a string raises TypeError
        self.assertRaises(TypeError, ds.mat_dist, data["eff_df"], met = 5)
        # Check if input is not a Dataframe raises TypeError
        self.assertRaises(ValueError, ds.mat_dist, data["eff_df"], met = "not_metric")
    def test_compare_df1_to_df2(self):
        ...
        comp = ds.compare_df1_to_df2(clust_df1 = df1, clust_df2 = df2, lab1 = lab1, lab2 = lab2)
